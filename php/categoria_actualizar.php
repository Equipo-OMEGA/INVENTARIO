<?php
	// Incluye el archivo "main.php" para tener acceso a las funciones y recursos necesarios
    ...
    // Verifica si la consulta no devuelve resultados (la categoría no existe)
    if ($check_categoria->rowCount() <= 0) {
    	...
    	// Almacena los datos de la categoría en la variable $datos
    ...
    // Parámetros a ser reemplazados en la consulta preparada
    $marcadores = [
        ":nombre" => $nombre,
        ":ubicacion" => $ubicacion,
        ":id" => $id
    ...
    if ($actualizar_categoria->execute($marcadores)) {
        ...
        echo '
            <div class="notification is-danger is-light">
                <strong>¡Ocurrió un error inesperado!</strong><br>
                No se pudo actualizar la categoría, por favor inténtelo nuevamente
            </div>
        ';
    }
